...
const CompanySizePage: React.FC = () => {
    ...
    const goToNextPage = async () => {
      ...
      if (clickedLabels.length > 0 && clickedLabels.length < 2) {
        console.log(clickedLabels.length);
        try {
          const response = await axios.post(`http://localhost:5000/survey/company-size/${uuid}`, {
          ...
          navigate(`/survey/industries-excited-in/${uuid}`);
        } catch (error) {
          console.error(`Error: ${error}`);
        }
      } else if (clickedLabels.length > 1) {
        notify("You can't select more than 1");
      } else {
